...
void print_table(int column_start,
                 int row_start) { // printing the table including the shift
  int m = sheet.rows;
  int n = sheet.cols;
...
queue *topological_sort(avl_tree *tree) {
  ...
  while (!is_empty(q)) {
    ...
    dependency_node *curr =
        sheet.data[node->data % 1000][node->data / 1000].dependencies;
    while (curr != NULL) {
