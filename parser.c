...
void parser(char *command)
{
    ...
    {
        // Allow optional spaces in the command format
        if (sscanf(command, "scroll_to %[A-Z]%d", ref_col, &ref_row) == 2 ||
            sscanf(command, "scroll_to%[A-Z]%d", ref_col, &ref_row) == 2)
        {
            ...
            {
                strcpy(status, "Invalid coordinates");
            }
        }
        else
        {
